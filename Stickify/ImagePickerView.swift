...
class ImagePickerView: UIViewController, UINavigationControllerDelegate {
    ...
    @IBAction func showMenu(_ sender: UIBarButtonItem) {
        let title = ActionSheetTitle(title: "Stickify  -  v1.2")

        let item0 = ActionSheetItem(title: "Send Feedback", image: UIImage(named: "feedback"))
        let item1 = ActionSheetItem(title: "Buy me a coffee (1,99€)", image: UIImage(named: "coffe"))
        ...
        ActionSheet(items: items) { _, item in
    ...
    @IBAction func openUrl(_ sender: UIBarButtonItem) {
        ...
        UIAlertController.show(vc: self, state: state, completion: { (result) in
            ...
            UIAlertController.show(vc: self, state: state, completion: { _ in })
        })
    }
    ...
    override func viewDidLoad() {
        ...
        IAPHandler.shared.purchaseStatusBlock = { (type) in
            if type == .purchased {
                ...
                APESuperHUD.showOrUpdateHUD(icon: image, message: "Donation success", particleEffectFileName: "FireFliesParticle", presentingView: self.view, completion: { })
            }
        }
    }
        ...
        if let vc = self.presentedViewController {
            ...
            for (index, v) in views.enumerated() {
                if index == 1 {
                    ...
                    observer = v.layer.observe(\.bounds) { _, _ in
...
extension UIAlertController {
    ...
    public class func show(vc: UIViewController, state: AlertState, completion: @escaping (String?) -> Void) {
        ...
        if let buttonA = state.normal {
            alert.addAction(UIAlertAction(title: buttonA, style: .default, handler: { _ in
                if state.textField {
